...
export class ToolHandlers {
  ...
  /**
   * Handle create_notebook_browser tool
   */
  async handleCreateNotebook(
    ...
    const { name, description, topics, show_browser } = args;
    log.info(`🔧 [TOOL] create_notebook_browser called: ${name}`);

    try {
      // 1. Get or create session (headless = !show_browser)
      await sendProgress?.("Initializing browser session...", 1, 5);
      const session = await this.sessionManager.getOrCreateSession(
      ...
      // 2. Create in browser
      await sendProgress?.("Creating notebook in browser...", 2, 5);
      const url = await session.createNotebook(name, sendProgress);

      // 3. Add to local library
      await sendProgress?.("Registering notebook in local library...", 4, 5);
      const notebook = this.library.addNotebook({
        url,
        name,
        description,
        topics,
        content_types: ["automated-creation"],
      });

      log.success(`✅ [TOOL] create_notebook_browser completed: ${url}`);
      return {
        success: true,
        data: { notebook },
      };
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      log.error(`❌ [TOOL] create_notebook_browser failed: ${errorMessage}`);
      return {
        success: false,
        error: errorMessage,
      };
  ...
  async handleAddSource(
    ...
    const { notebook_id, notebook_url, source_path, show_browser } = args;
    log.info(`🔧 [TOOL] add_source called: ${source_path}`);

    try {
      // Resolve URL
      let url = notebook_url;
      if (!url && notebook_id) {
        url = this.library.getNotebook(notebook_id)?.url;
      }
      if (!url) {
        url = this.library.getActiveNotebook()?.url;
      }
      if (!url) throw new Error("No notebook selected or URL provided");
      ...
      // 2. Add source
      await session.addSource(source_path, sendProgress);

      return {
        success: true,
        data: { status: "Source added successfully" },
      };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
  ...
  /**
   * Handle update_instructions tool
   */
  async handleUpdateInstructions(
    args: {
      notebook_id?: string;
      notebook_url?: string;
      instructions: string;
    ...
    const { notebook_id, notebook_url, instructions, show_browser } = args;
    log.info(`🔧 [TOOL] update_instructions called`);
    ...
    try {
      ...
      await session.updateInstructions(instructions, sendProgress);

      return {
        success: true,
        data: { status: "Instructions updated successfully" },
      };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      log.error(`❌ [TOOL] update_instructions failed: ${errorMessage}`);
      return {
        success: false,
        error: errorMessage,
      };
  ...
  async handleRemoveSource(
    ...
    const { notebook_id, notebook_url, source_name, show_browser } = args;
    log.info(`🔧 [TOOL] remove_source called: ${source_name}`);
    ...
    try {
      ...
      // 2. Remove source
      await session.removeSource(source_name, sendProgress);

      return {
        success: true,
        data: { status: "Source removed successfully" },
      };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
