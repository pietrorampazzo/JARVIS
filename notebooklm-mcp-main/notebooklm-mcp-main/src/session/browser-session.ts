...
import { humanType, randomDelay, realisticClick } from "../utils/stealth-utils.js";
import {
  waitForLatestAnswer,
...
export class BrowserSession {
   ...
   */
  async createNotebook(name: string, sendProgress?: ProgressCallback): Promise<string> {
    const createOnce = async (): Promise<string> => {
      if (!this.initialized || !this.page || this.isPageClosedSafe()) {
        await this.init();
      }

      log.info(`🆕 [${this.sessionId}] Creating notebook: "${name}"`);
      const page = this.page!;

      // 1. Go to home page if not already there
      const homeUrl = "https://notebooklm.google.com/";
      if (!page.url().startsWith(homeUrl)) {
        await sendProgress?.("Navigating to NotebookLM home...", 1, 5);
        await page.goto(homeUrl, { waitUntil: "domcontentloaded" });
        await randomDelay(2000, 3000);
      }

      // 2. Click "New Notebook"
      await sendProgress?.("Clicking 'New Notebook'...", 2, 5);
      const createButtonSelector = "mat-card.create-new-action-button, button.create-new-button";
      await page.waitForSelector(createButtonSelector, { state: "visible", timeout: 15000 });
      await realisticClick(page, createButtonSelector);

      // 3. Wait for the new notebook to load
      await sendProgress?.("Waiting for new notebook to initialize...", 3, 5);
      await page.waitForURL(url => url.toString().includes("notebook/"), { timeout: 30000 });
      const newUrl = page.url();

      // 4. Update the name if possible (NotebookLM often gives a default name)
      // For now, we'll just return the URL, and maybe rename later if we find the selector

      this.updateActivity();
      log.success(`✅ [${this.sessionId}] Notebook created: ${newUrl}`);
      return newUrl;
    };
    ...
    try {
      ...
      log.error(`❌ [${this.sessionId}] Failed to create notebook: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  async addSource(filePathOrUrl: string, sendProgress?: ProgressCallback): Promise<void> {
    if (!this.initialized || !this.page || this.isPageClosedSafe()) {
      await this.init();
    }

    log.info(`📥 [${this.sessionId}] Adding source: "${filePathOrUrl}"`);
    const page = this.page!;

    try {
      // 1. Click "Add source"
      await sendProgress?.("Opening 'Add source' menu...", 1, 5);
      const addSourceSelector = "button.add-source-button, button[aria-label*='Adicionar fonte'], button[aria-label*='Add source']";
      await page.waitForSelector(addSourceSelector, { state: "visible" });
      await realisticClick(page, addSourceSelector);
      await randomDelay(1000, 2000);

      // 2. Handle different types of sources
      if (filePathOrUrl.startsWith("http")) {
        // Link/Website
        await sendProgress?.("Selecting 'Website/Link' source type...", 2, 5);
        // This part needs more specific selectors for the modal
      } else {
        // File upload
        await sendProgress?.("Selecting 'File upload' source type...", 2, 5);
        const fileInputSelector = "input[type='file']";
        // NotebookLM might use a hidden input or a specific button
        // For now, let's assume we can find the file input
        await page.setInputFiles(fileInputSelector, filePathOrUrl);
      }

      await sendProgress?.("Uploading source...", 4, 5);
      // Wait for upload to complete
      await randomDelay(5000, 10000);

      this.updateActivity();
      log.success(`✅ [${this.sessionId}] Source added`);
    } catch (error: any) {
      log.error(`❌ [${this.sessionId}] Failed to add source: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  async updateInstructions(instructions: string, sendProgress?: ProgressCallback): Promise<void> {
    if (!this.initialized || !this.page || this.isPageClosedSafe()) {
      await this.init();
    }

    log.info(`📝 [${this.sessionId}] Updating instructions...`);
    const page = this.page!;

    try {
      // 1. Open settings
      await sendProgress?.("Opening notebook settings...", 1, 5);
      const settingsSelector = "button.configure-settings-button, button[aria-label*='Configurar'], button[aria-label*='Configure']";
      await page.waitForSelector(settingsSelector, { state: "visible" });
      await realisticClick(page, settingsSelector);
      await randomDelay(1000, 2000);

      // 2. Find instructions textarea
      await sendProgress?.("Typing instructions...", 3, 5);
      const instructionsSelector = "textarea[aria-label*='instruções'], textarea[aria-label*='instructions']";
      await page.waitForSelector(instructionsSelector, { state: "visible" });

      await humanType(page, instructionsSelector, instructions);
      await randomDelay(1000, 2000);

      // 3. Close settings (auto-saves usually)
      await page.keyboard.press("Escape");

      this.updateActivity();
      log.success(`✅ [${this.sessionId}] Instructions updated`);
    } catch (error: any) {
      log.error(`❌ [${this.sessionId}] Failed to update instructions: ${error.message}`);
      throw error;
    }
  }

  /**
   * Remove a source from the current notebook
   */
  async removeSource(sourceName: string, sendProgress?: ProgressCallback): Promise<void> {
    if (!this.initialized || !this.page || this.isPageClosedSafe()) {
      await this.init();
    }

    log.info(`🗑️ [${this.sessionId}] Removing source: "${sourceName}"`);
    const page = this.page!;

    try {
      // 1. Find the source item more button
      await sendProgress?.(`Looking for source "${sourceName}"...`, 1, 5);

      // We need to find the container that has the source name and then click its 'more' button
      const sourceSelector = `.source-item-container:has-text("${sourceName}"), [role="listitem"]:has-text("${sourceName}")`;
      const sourceItem = await page.$(sourceSelector);

      if (!sourceItem) {
        throw new Error(`Source "${sourceName}" not found in the list`);
      }

      await sendProgress?.("Opening context menu...", 2, 5);
      const moreButton = await sourceItem.$("button.source-item-more-button, button[aria-label*='mais'], button[aria-label*='more']");
      if (!moreButton) throw new Error("Could not find 'more' button for source");

      await moreButton.click();
      await randomDelay(1000, 2000);

      // 2. Click Delete
      await sendProgress?.("Clicking delete...", 4, 5);
      const deleteSelector = "button.more-menu-delete-source-button, [role='menuitem']:has-text('Excluir'), [role='menuitem']:has-text('Delete')";
      await page.waitForSelector(deleteSelector, { state: "visible" });
      await realisticClick(page, deleteSelector);

      // 3. Confirm if modal appears
      await randomDelay(1000, 2000);
      const confirmSelector = "button:has-text('Confirmar'), button:has-text('Delete'), button.confirm-button";
      if (await page.$(confirmSelector)) {
        await page.click(confirmSelector);
      }

      this.updateActivity();
      log.success(`✅ [${this.sessionId}] Source removed`);
    } catch (error: any) {
      log.error(`❌ [${this.sessionId}] Failed to remove source: ${error.message}`);
      throw error;
    }
  }
