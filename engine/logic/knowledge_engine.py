...
from engine.shared.shared import get_config, get_today_log, load_json, LOGS_DIR, DIA_MD_PATH, HEARTBEAT_FILE, get_latest_snapshot
...
def update_snapshots():
    ...
    projects_config = get_config("projects")
    heartbeat_data = load_json(HEARTBEAT_FILE)
...
def collect_strategic_decisions():
    ...
    for event in today_events:
        if event.get("category", "").upper() == "DECISION":
            ...
            local_path = proj_path / "records" / "decisions"
...
def update_strategies():
    ...
    for p in projects_config.get("projects", []):
        ...
        proj_id = target_mapping[proj_id_raw]
        local_decision_file = Path(p["path"]) / "records" / "decisions" / "decisions.md"
