...
from engine.shared.shared import get_config, get_today_log, load_json, LOGS_DIR, get_latest_snapshot, load_env
from engine.logic.score_engine import calculate_daily_score
from engine.oracle.oracle_query import query_oracle
...
- REGRAS DE NEGÓCIO ARTE (Crucial para seus insights):
  * "DADOS": IGNORE esta coluna. Ela não faz parte do funil de análise operacional.
  ...
  * "HABILITADO": Representa uma VITÓRIA. Não é um gargalo de formalização. Deve ser comparado com o volume anterior (ex: ontem tínhamos 45, hoje 48). Se aumentou, parabenize o usuário (+15 pts por unidade).
  * "RATE_LIMITED": Se encontrar este termo, explique que são itens que o NotebookLM não conseguiu processar por excesso de requisições e precisam de reprocessamento em outra sessão.
...
- Sobre os HABILITADOS: Trate como métrica de sucesso de vendas. Se o número subiu, o dia foi produtivo.
...
def build_dia_md():
    now = datetime.now()
    # Pula o inventory aqui para evitar duplicidade com o jarvis_pulse que já rodou
    score = calculate_daily_score(skip_inventory=True)
    logs = get_today_log()
...
"""
    ...
    import sys
    sys.path.insert(0, str(BASE_DIR / "engine" / "monitoring"))
    try:
