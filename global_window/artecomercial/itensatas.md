...
import { Package, Search, Filter, RotateCcw } from"lucide-react";

exportdefaultfunctionItensAtasPage() {
  ...
  const filteredItens = itens.filter(item => {
    ...
    const categoriaMatch = categoriaFilter === "all" || item.categoria === categoriaFilter;
  ...
  const formatCurrency = (value) => {
    ...
    returnnewIntl.NumberFormat('pt-BR', {
  ...
  return (
    ...
    `<div className="text-green-100 text-sm font-medium mb-2">`ValorTotalRegistrado`</div>`
    ...
    `<div className="text-purple-100 text-sm font-medium mb-2">`QuantidadeTotal`</div>`
    ...
    `<SelectItem value="all">`TodasCategorias`</SelectItem>`
    ...
    `<SelectItem value="all">`TodosStatus`</SelectItem>`
