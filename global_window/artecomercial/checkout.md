...
exportdefaultfunctionCheckoutPage() {
  ...
  if (enviado) {
    ...
    `<a href="tel:+5511994103374" className="text-blue-600 hover:underline">`
  ...
  if (itens.length === 0) {
    ...
    `<h2 className="text-3xl font-bold text-gray-900 mb-4">`CarrinhoVazio`</h2>`
  ...
  return (
    ...
    FinalizarSolicitação de Cotação
    ...
    `<Label htmlFor="nome_completo">`NomeCompleto *`</Label>`
    ...
    `<Label htmlFor="cnpj">`CNPJEmpresa/Órgão (UASG) *`</Label>`
    ...
    `<span>`Qtd: {item.quantidade}

    {item.valor_unitario && (

    `<span>`{formatCurrency(item.valor_unitario * item.quantidade)}
    ...
    `<span className="text-gray-600">`Total de itens:

    `<span className="font-medium">`{itens.reduce((t, i) => t + i.quantidade, 0)}
    ...
    `<span className="text-blue-600">`{formatCurrency(valorTotal)}
